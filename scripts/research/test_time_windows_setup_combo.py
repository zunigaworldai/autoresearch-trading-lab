...
DEFAULT_WINDOWS = [
    ("open_0930_1000", "09:30", "10:00"),
    ("open_0930_1030", "09:30", "10:30"),
    ("open_0930_1100", "09:30", "11:00"),
...
def build_params(setup_combo: str, start: str, end: str) -> dict[str, Any]:
    if setup_combo not in SETUP_MAP:
        raise SystemExit(f"ERROR: invalid setup_combo={setup_combo}. Valid: {sorted(SETUP_MAP)}")

    enable_a, enable_b, enable_c = SETUP_MAP[setup_combo]
    params = DEFAULT_PARAMS.copy()
    params["use_rth"] = True
    params["session_start"] = start
    params["session_end"] = end
...
def classify(row: dict[str, Any], min_trades: int) -> str:
    ...
    if row["trades_per_calendar_month"] < 2:
...
def parse_window(raw: str) -> tuple[str, str, str]:
    ...
    if len(parts) != 3:
        raise SystemExit("ERROR: --window format must be label,start,end")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    windows = [parse_window(w) for w in args.window] if args.window else DEFAULT_WINDOWS
    setup_combos = args.setup_combos

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    rows = []
    for setup_combo in setup_combos:
        for window_label, start, end in windows:
            params = build_params(setup_combo, start, end)
    ...
    result = result.sort_values(
        ["cost_label", "window_score", "pf", "expectancy", "trades"],
        ascending=[True, False, False, False, False],
    ).reset_index(drop=True)

    safe = f"{args.symbol.upper()}_{args.timeframe}_{args.exit_policy}_time_windows_setup_combo"
    ...
    report = {
        ...
        "exit_policy": args.exit_policy,
        "csv": args.csv,
        "setup_combos": setup_combos,
        "windows_analyzed": [{"label": w[0], "start": w[1], "end": w[2]} for w in windows],
        "rows": int(len(result)),
        ...
        "warning": "Window search is diagnostic only. Validate selected windows out-of-sample and portfolio-level before paper/live.",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Test intraday windows by setup_combo and exit_policy on 5Y data.")
    parser.add_argument("--symbol", required=True)
    parser.add_argument("--timeframe", required=True)
    parser.add_argument("--csv", required=True)
    parser.add_argument("--exit-policy", default="partial_50_at_1r_be", choices=sorted(VALID_EXIT_POLICIES))
    parser.add_argument("--setup-combos", nargs="+", default=["A_only", "A_C", "A_B", "A_B_C"])
    parser.add_argument("--window", action="append", default=None, help="Optional custom window: label,start,end")
    parser.add_argument("--min-trades", type=int, default=40)
    parser.add_argument("--top-n", type=int, default=25)
    parser.add_argument("--output-dir", default="outputs/research/time_windows_setup_combo")
