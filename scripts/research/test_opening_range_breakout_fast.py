...
def evaluate_trades(
    ...
    row = {
        "symbol": symbol,
        "timeframe": timeframe,
        "strategy_family": "opening_range_breakout_fast",
        "variant": variant,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if unknown_windows:
        raise SystemExit(f"ERROR: unknown entry windows {unknown_windows}. Valid: {sorted(ENTRY_WINDOWS)}")
    ...
    safe = f"{args.symbol.upper()}_{args.timeframe}_{args.exit_policy}_opening_range_breakout_fast"
    ...
    report = {
        ...
        "warning": "Fast diagnostic only. Must pass full cost stress, OOS, walk-forward, and portfolio checks before paper/live.",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fast Opening Range Breakout diagnostic runner.")
    parser.add_argument("--symbol", required=True)
    parser.add_argument("--timeframe", required=True)
    parser.add_argument("--csv", required=True)
    parser.add_argument("--exit-policy", default="partial_50_at_1r_be", choices=sorted(VALID_EXIT_POLICIES))
    parser.add_argument("--or-minutes", nargs="+", type=int, default=[10, 15, 30])
    parser.add_argument(
        "--variants",
        nargs="+",
        default=["both_or_stop_rr15_vol", "both_or_stop_rr2", "long_or_stop_rr2"],
        choices=sorted(VARIANTS),
    )
    parser.add_argument(
        "--entry-windows",
        nargs="+",
        default=["entry_0945_1030", "entry_1000_1130", "entry_1030_1200", "entry_1100_1300"],
    ...
    parser.add_argument("--output-dir", default="outputs/research/opening_range_breakout_fast")
