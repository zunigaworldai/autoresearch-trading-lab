...
def run_setup_window_cost(
    ...
    if save_artifacts:
        ...
        trades_path = str(output_dir / f"{base}_trades.csv")
        equity_path = str(output_dir / f"{base}_equity.csv")
...
def run_experiment(
    ...
    rows = []
    total = len(SETUP_COMBINATIONS) * len(windows) * len(COST_SCENARIOS)
    idx = 0
    ...
    for setup_label, enable_a, enable_b, enable_c in SETUP_COMBINATIONS:
        for session_start, session_end, window_label in windows:
            for cost_label, cost_per_side in COST_SCENARIOS.items():
                idx += 1
                print(
                    f"[{symbol} {timeframe}] {idx}/{total} "
                    f"{setup_label} {window_label} {cost_label}",
                    flush=True,
                    file=sys.stderr,
                )
                rows.append(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run setup-combination experiments fast.")
    parser.add_argument("--symbol", required=True)
    parser.add_argument("--timeframe", required=True)
    parser.add_argument("--csv", required=True)
    parser.add_argument("--exit-policy", default="break_even_1r", choices=sorted(VALID_EXIT_POLICIES))
    parser.add_argument("--output-dir", default="outputs/research/setup_combos")
    parser.add_argument("--window", action="append", default=None)
    parser.add_argument("--min-trades-for-candidate", type=int, default=20)
    parser.add_argument(
        "--save-artifacts",
        action="store_true",
        help="Save per-combo trades/equity CSV files. Default is summary-only for speed.",
    )
